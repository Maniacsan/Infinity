...
public abstract class ListWidget extends AbstractParentElement implements Drawable {
	...
	protected abstract void renderItem(MatrixStack matrixStack, int x, int y, int itemHeight, int mouseX, int mouseY,
			int i, float f);
	...
	public int getItemAtPosition(double mouseX, double mouseY) {
		...
		int k = MathHelper.floor(mouseY - top) - headerHeight + (int) scrollAmount - 4;
		int l = k / itemHeight;
		return mouseX < getScrollbarPosition() && mouseX >= i && mouseX <= j && l >= 0 && k >= 0 && l < getItemCount()
				? l
				: -1;
	}
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		if (visible) {
			...
			bufferBuilder.vertex(left, bottom, 0.0D).texture(left / 32.0F, (bottom + (int) scrollAmount) / 32.0F)
					.color(32, 32, 32, 255).next();
			bufferBuilder.vertex(right, bottom, 0.0D).texture(right / 32.0F, (bottom + (int) scrollAmount) / 32.0F)
					.color(32, 32, 32, 255).next();
			bufferBuilder.vertex(right, top, 0.0D).texture(right / 32.0F, (top + (int) scrollAmount) / 32.0F)
					.color(32, 32, 32, 255).next();
			bufferBuilder.vertex(left, top, 0.0D).texture(left / 32.0F, (top + (int) scrollAmount) / 32.0F)
					.color(32, 32, 32, 255).next();
			tessellator.draw();
			...
			int n = getMaxScroll();
			if (n > 0) {
				int o = (int) ((float) ((bottom - top) * (bottom - top)) / (float) getMaxPosition());
				...
				bufferBuilder.vertex(i, bottom, 0.0D).color(0, 0, 0, 255).next();
				bufferBuilder.vertex(j, bottom, 0.0D).color(0, 0, 0, 255).next();
				bufferBuilder.vertex(j, top, 0.0D).color(0, 0, 0, 255).next();
				bufferBuilder.vertex(i, top, 0.0D).color(0, 0, 0, 255).next();
				tessellator.draw();
				bufferBuilder.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
				bufferBuilder.vertex(i, p + o, 0.0D).color(128, 128, 128, 255).next();
				bufferBuilder.vertex(j, p + o, 0.0D).color(128, 128, 128, 255).next();
				bufferBuilder.vertex(j, p, 0.0D).color(128, 128, 128, 255).next();
				bufferBuilder.vertex(i, p, 0.0D).color(128, 128, 128, 255).next();
				tessellator.draw();
				bufferBuilder.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
				bufferBuilder.vertex(i, p + o - 1, 0.0D).color(192, 192, 192, 255).next();
				bufferBuilder.vertex(j - 1, p + o - 1, 0.0D).color(192, 192, 192, 255).next();
				bufferBuilder.vertex(j - 1, p, 0.0D).color(192, 192, 192, 255).next();
				bufferBuilder.vertex(i, p, 0.0D).color(192, 192, 192, 255).next();
				tessellator.draw();
			}
	...
	protected void updateScrollingState(double mouseX, double mouseY, int button) {
		scrolling = button == 0 && mouseX >= getScrollbarPosition() && mouseX < getScrollbarPosition() + 6;
	}
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		if (isVisible() && isMouseInList(mouseX, mouseY)) {
			...
			if (i == -1 && button == 0) {
				clickedHeader((int) (mouseX - (left + width / 2 - getRowWidth() / 2)),
	...
	public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
		if (super.mouseDragged(mouseX, mouseY, button, deltaX, deltaY))
			...
			if (mouseY < top)
				...
				int i = (int) ((float) ((bottom - top) * (bottom - top)) / (float) getMaxPosition());
	...
	protected void renderHoleBackground(int top, int bottom, int topAlpha, int bottomAlpha) {
		...
		RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
		bufferBuilder.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
		bufferBuilder.vertex(left, bottom, 0.0D).texture(0.0F, bottom / 32.0F).color(64, 64, 64, bottomAlpha).next();
		bufferBuilder.vertex(left + width, bottom, 0.0D).texture(width / 32.0F, bottom / 32.0F)
				.color(64, 64, 64, bottomAlpha).next();
		bufferBuilder.vertex(left + width, top, 0.0D).texture(width / 32.0F, top / 32.0F).color(64, 64, 64, topAlpha)
				.next();
		bufferBuilder.vertex(left, top, 0.0D).texture(0.0F, top / 32.0F).color(64, 64, 64, topAlpha).next();
		tessellator.draw();
	}
}