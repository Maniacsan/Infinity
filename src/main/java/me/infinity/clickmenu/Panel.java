		...
		for (CategoryButton catButton : catButton) {
			if (catButton.displayModulePanel) {
				if (catButton.getName() == "CONFIGS") {
					...
					Render2D.drawRectWH(matrices, x + 69, y + 3, 77, height - 6, ColorUtils.backGroundLine);
					...
					Render2D.drawRectWH(matrices, x + 313, y + 3, 64, height - 6, ColorUtils.backGroundLine);
				} else {
				Render2D.drawRectWH(matrices, x + 68, y + 2, 140, height - 4, ColorUtils.lineColor);
				Render2D.drawRectWH(matrices, x + 69, y + 3, 138, height - 6, ColorUtils.backGroundLine);
				}
			}
