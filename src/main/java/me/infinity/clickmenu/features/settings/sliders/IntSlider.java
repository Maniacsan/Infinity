...
public class IntSlider extends SettingButton {
	...
	public IntSlider(Settings setting) {
		super(setting);
		this.selected = setting.getCurrentValueInt() / setting.getMaxValueInt();
	}
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta, double x, double y, double width,
		...
		FontUtils.drawHVCenteredString(matrices, setting.isValueInt() ? intVal : intVal, (float) (x + width + 17),
				(float) (y + 14.0), -1);
		...
		if (this.dragging) {
			final double diff = setting.getMaxValueInt() - setting.getMinValueInt();

			final double percentBar = MathHelper.clamp((mouseX - x) / width, 0.0, 1.0);
			final double val = setting.getMinValueInt() + percentBar * diff;
